package club.slavopolis.jdbc.core;

import club.slavopolis.common.response.PageResult;
import club.slavopolis.jdbc.parameter.TypeSafeParameterSource;
import club.slavopolis.jdbc.transaction.TransactionCallback;
import club.slavopolis.jdbc.transaction.TransactionStatus;
import club.slavopolis.jdbc.transaction.DefaultTransactionStatus;
import club.slavopolis.jdbc.mapping.IntelligentRowMapper;
import club.slavopolis.jdbc.monitoring.SqlExecutionMonitor;
import club.slavopolis.jdbc.security.SqlInjectionValidator;
import club.slavopolis.jdbc.security.SqlSecurityConfig;
import club.slavopolis.jdbc.exception.TransactionException;
...
/**
 * 增强JDBC模板实现类
 * <p>基于NamedParameterJdbcTemplate的深度封装，提供企业级数据访问能力</p>
 * <p>支持类型安全参数绑定、智能结果映射、编程式事务管理、SQL安全防护等功能</p>
 *
 * <h3>SQL安全性保障</h3>
 * <p>本类通过多层安全防护机制确保SQL执行安全：</p>
 * <ul>
 *     <li><strong>SQL注入防护</strong>：使用SqlInjectionValidator进行SQL注入检测</li>
 *     <li><strong>参数化查询</strong>：基于NamedParameterJdbcTemplate确保所有查询都是参数化的</li>
 *     <li><strong>参数验证</strong>：对所有输入参数进行安全性验证</li>
 *     <li><strong>操作审计</strong>：记录所有SQL操作用于安全审计</li>
 *     <li><strong>敏感信息掩码</strong>：日志中的敏感信息自动掩码处理</li>
 * </ul>
 *
 * <p><strong>注意</strong>：@SuppressWarnings("SqlSourceToSinkFlow") 注解的使用是安全的，
 * 因为所有SQL字符串都经过了严格的安全验证流程。</p>
 *
...
public class EnhancedJdbcTemplate implements JdbcOperations {
    ...
    public EnhancedJdbcTemplate(NamedParameterJdbcTemplate namedParameterJdbcTemplate,
        ...
        Assert.notNull(namedParameterJdbcTemplate, "NamedParameterJdbcTemplate must not be null");
        Assert.notNull(transactionManager, "DataSourceTransactionManager must not be null");
        this.namedParameterJdbcTemplate = namedParameterJdbcTemplate;
    ...
    public EnhancedJdbcTemplate(NamedParameterJdbcTemplate namedParameterJdbcTemplate,
        ...
        Assert.notNull(transactionManager, "DataSourceTransactionManager must not be null");
        Assert.notNull(transactionDefinition, "TransactionDefinition must not be null");
        Assert.notNull(securityConfig, "SqlSecurityConfig must not be null");
        this.namedParameterJdbcTemplate = namedParameterJdbcTemplate;
        this.transactionManager = transactionManager;
        this.defaultTransactionDefinition = transactionDefinition;
        this.executionMonitor = new SqlExecutionMonitor();
    ...
    public <T> List<T> queryForList(String sql, Map<String, Object> params, RowMapper<T> rowMapper) {
        return executeQuery(sql, params, defaultLoggingEnabled, () ->
    ...
    public int update(String sql, Map<String, Object> params, boolean enableLogging) {
        return executeUpdate(sql, params, enableLogging, () ->
    ...
    public Number updateAndReturnKey(String sql, TypeSafeParameterSource paramSource, boolean enableLogging) {
        return executeUpdate(sql, Objects.requireNonNull(paramSource.getParameterNames()).length > 0 ?
                Arrays.stream(paramSource.getParameterNames())
                        .collect(HashMap::new, (map, name) -> map.put(name, paramSource.getValue(name)), HashMap::putAll) :
                new HashMap<>(), enableLogging, () -> {
    ...
    public TransactionStatus beginTransaction() {
        ...
        try {
            org.springframework.transaction.TransactionStatus springStatus =
    ...
    @Override
    public PageResult.PageData<Map<String, Object>> queryForPage(String sql, Map<String, Object> params, int pageNum, int pageSize) {
    ...
    @Override
    @SuppressWarnings("SqlSourceToSinkFlow")
    public <T> PageResult.PageData<T> queryForPage(String sql, Map<String, Object> params, Class<T> requiredType, int pageNum, int pageSize) {
        validatePageParams(pageNum, pageSize);
    ...
    public long queryForCount(String sql, Map<String, Object> params) {
        return executeQuery(sql, params, defaultLoggingEnabled, () -> {
            Long result = namedParameterJdbcTemplate.queryForObject(sql, params, Long.class);
     ...
     * @param params 参数映射
     * @return 验证后的SQL语句
     * @throws club.slavopolis.jdbc.exception.SecurityException 如果SQL不安全
     */
